export default function LovableLogo({ className = 'w-6 h-6', showText = false, textClassName = '' }) {
  return (
    <span className={`inline-flex items-center gap-2`}>
      {/* Official Lovable Logo */}
      <svg viewBox="0 0 121 122" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
        <mask
          id="mask0_lovable"
        ...
        <defs>
          ...
          >
            ...
            <feGaussianBlur stdDeviation="18.1936" result="effect1_foregroundBlur" />
          </filter>
          <filter
            id="filter1_lovable"
            x="-78.9301"
      ...
      {showText && <span className={`font-semibold ${textClassName || 'text-white'}`}>Lovable</span>}
    </span>
  );
}
